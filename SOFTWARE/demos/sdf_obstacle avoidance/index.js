...
canvas.addEventListener("mousemove", ev => {
	...
	for (let i=0; i<rects.length; i+=8){
		...
		for (let n=2; n < 8; n+=2) ctx.lineTo(rects[i+n], rects[i+n+1]);
		ctx.closePath();
		ctx.stroke();
	...
	for (let polygon of polygons){
		...
		for (let i=0; i<polygon.length; i+=4){
			...
			ctx.lineTo(polygon[i+2], polygon[i+3]);
			ctx.stroke();
			
			let linePntDir = [pos[0]-polygon[i+0], pos[1]-polygon[i+1]];
			let lineAxis = [polygon[i+2]-polygon[i+0], polygon[i+3]-polygon[i+1]];
			let lineSize = Math.sqrt( lineAxis[0]**2 + lineAxis[1]**2 );
			lineAxis[0] /= lineSize; lineAxis[1] /= lineSize;
			let projPos = linePntDir[0]*lineAxis[0] + linePntDir[1]*lineAxis[1];
			let local_orig = clamp(0, lineSize, projPos);
			let orig = [
				polygon[i+0]+lineAxis[0]*local_orig,
				polygon[i+1]+lineAxis[1]*local_orig
			];
			
			let lineDir = [pos[0]-orig[0], pos[1]-orig[1]];
			let lineDist = Math.sqrt( lineDir[0]**2 + lineDir[1]**2 );
			if (lineDist >= mindist) continue;
			lineDir[0] /= lineDist; lineDir[1] /= lineDist;
		...
		ctx.fillRect(minorig[0]-3, minorig[1]-3, 6, 6);
	}
	ctx.beginPath();
